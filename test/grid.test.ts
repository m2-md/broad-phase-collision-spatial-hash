...
function scene(n: number, seed: number): TestItem[] {
  ...
  return Array.from({ length: n }, (_, id) => ({
    ...
    radius: 6 + rand() * 10, // varied sizes: stress multi-cell coverage of AABBs
  }));
}

// Brute-force: all truly overlapping pairs (ground-truth reference)
...
describe("SpatialHashGrid.queryPairs", () => {
  it("does not miss any truly overlapping pairs (identical to brute-force)", () => {
  ...
  it("does not produce the same pair twice (no double counting)", () => {
  ...
  it("matches large entity spanning multiple cells with all neighbors", () => {
    ...
    const grid = new SpatialHashGrid<TestItem>(24); // cell size << large diameter
    ...
    expect(hits).toEqual(new Set([1, 2, 3])); // caught all three
  });
});
